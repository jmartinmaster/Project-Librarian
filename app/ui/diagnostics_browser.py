...
class DiagnosticsBrowser(QWidget):
    ...
    def _init_service(self) -> None:
        try:
            ...
            # Grab a base reference point automatically
            self.service.profile_memory_payload("start")
            res = self.service.profile_memory_payload("take_snapshot")
            if res.get("ok") and "total_allocated_kb" in res:
                self.memory_history.append((res.get("snapshot_name", "base_snapshot"), res["total_allocated_kb"]))
    ...
    def take_snapshot(self) -> None:
        ...
        output = [
            f"Snapshot recorded: {snap_name}",
            f"Total Allocated Memory: {size_kb} KB",
            "",
            "Top allocations:",
    ...
    def run_leak_test(self) -> None:
        ...
        msg = f"Starting profiling for: {target_path}...\n(Duration: {duration}s, Interval: {interval}s, Headless: {headless})\n(Please wait, profiling in a background process)" if target_path else "Starting headless memory profiling...\n(Please wait, profiling IndexManager in a background process)"
        ...
        self.runner = self.service.run_subprocess_profile(target_path, duration=duration, interval=interval, headless=headless)
        self.runner.target_path = target_path
        ...
        def on_profiling_completed(result: dict):
            ...
            if result.get("error"):
                ...
                match = re.search(r"ModuleNotFoundError: No module named '([^']+)'", err_msg)
                if match:
                    module_name = match.group(1)
                    # Common mappings for import name to PyPI package name
                    package_mapping = {
                        "PIL": "Pillow",
                        "cv2": "opencv-python",
                        "yaml": "PyYAML",
                    ...
                    default_pkg = package_mapping.get(module_name, module_name)
