...
- **Embedded MVC Editor** – hosts a full-featured code editor directly inside The Librarian, with a workspace explorer, triad file discovery, inspector/sync navigation, run console, and Model/View/Controller sub-tabs for triad load and save.
...
- Platform-aware configuration storage:
  - **Windows:** `%LOCALAPPDATA%\The Librarian\` for packaged builds (otherwise `%APPDATA%\`)
...
When The Librarian starts with a fresh configuration, it uses the folder it was opened from as the initial index root. After you save a project root in settings, later launches reuse that saved location until you change it. On first run (when no snapshot exists yet), The Librarian builds the initial index and snapshot before launching the main window; subsequent refreshes run in the background so the UI stays responsive.
...
The Librarian is licensed under the GNU GPL v3.0 or later. See `LICENSE` for the full text.
