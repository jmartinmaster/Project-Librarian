...
"""Smoke tests for the AntiPatternBrowser widget."""
...
from app.ui.anti_pattern_browser import AntiPatternBrowser, PresetDialog


def test_anti_pattern_browser_loads_and_lists_defaults(qtbot, app_config):
    manager = IndexManager(app_config)
    manager.refresh()

    widget = AntiPatternBrowser(manager)
...
def test_anti_pattern_browser_scans_code(qtbot, app_config, sample_repo):
...
def test_anti_pattern_browser_adds_preset(monkeypatch, qtbot, app_config):
    manager = IndexManager(app_config)
    manager.refresh()
...
def test_anti_pattern_browser_deletes_preset(monkeypatch, qtbot, app_config):
    manager = IndexManager(app_config)
    manager.refresh()
...
def test_anti_pattern_browser_severity_coloring(qtbot, app_config, sample_repo):
...
def test_anti_pattern_browser_exports_csv(monkeypatch, qtbot, app_config, tmp_path):
    manager = IndexManager(app_config)
    manager.refresh()
