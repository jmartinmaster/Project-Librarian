...
def test_workspace_service_formats_git_summary(monkeypatch, app_config):
    manager = IndexManager(app_config)
    service = WorkspaceService(index_manager=manager)
...
def test_workspace_service_generates_docs_and_changelog(monkeypatch, app_config):
    manager = IndexManager(app_config)
